	...
	*/ 	
	var gmap = function(oMap, oMapJSON, oConfig, oTemplate) {
		...
		if (oMapContent && oMapJSON && oConfig) {
			...
			google.maps.event.addListener(oMap, "dragstart", function() {
    			unhideMarkers();
			});
		...
		*/ 	
		function setMarkers(sValue, sKey, bFitBoundsToMarkers, oReferrer, bSetAllMarkers) {
					...
					for (j = 0; j < aKeys.length; j++) {
						...
						if (oReferrer) {
							...
							if (oReferrer.getAttribute("data-search-type") == "json-contains") {
								setMarker(~sTmp.toLowerCase().indexOf(sValue.toLowerCase()), oMapJSON.data[i], nRanking, i);
							}
							/* JSON Equals */
							else {
								setMarker((sTmp == sValue), oMapJSON.data[i], nRanking, i);
							}
						}
						else { 
							/* Geocoder */
							setMarker((sTmp == sValue), oMapJSON.data[i], 0, i);
						}
					}
				}
			}
			...
			*/ 	
			function setMarker(bSetMarker, oJSON, nRanking, nIdx) {
				nIdx = !isNaN(nIdx) ? nIdx : -1;
				...
				if (bSetMarker) { 
					...
					if (oRanking.hasOwnProperty(sName)) {
						...
						if(nIdx > -1) {
							aMarkers[nIdx].setMap(oMap);
							aMarkersSearch.push(aMarkers[nIdx]);
							bSearch = true;
						} else {
			...
			if (oConfig.Custom.markerClusterer && ("MarkerClusterer" in window)) {
				oClusterer = new MarkerClusterer(oMap, (aMarkersSearch.length ? aMarkersSearch : aMarkers), oConfig.Clusterer);
